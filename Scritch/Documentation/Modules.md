...
___
*<p align=center>Modules are available in Scritch version 1.2.0 and above.</p>*
___

Just to be very clear before we get started: **Scritch does not support commonJS/node/ES6 modules**. It has a custom import system that may or may not be compatible with some existing modules.
...
Scritch ships with a couple of modules, with the prefix `@scritch/`, which you can use in your own scripts.

Scritch is a fork of [Boop](https://github.com/IvanMathy/Boop), and the older `@boop/` prefix still
resolves to the same modules, so scripts written for Boop work unmodified. Prefer `@scritch/` in new
scripts.

| Module name        | Description   |
| ------------------ | ------------- |
| `@scritch/base64`     | Base 64 encoding and decoding |
| `@scritch/lodash.boop`| Subset of lodash string functions (see below) |
| `@scritch/he`         | HTML entities encoder/decoder |
| `@scritch/vkBeautify` | XML, CSS, and SQL formatter and minifier |
| `@scritch/js-yaml`    | Parses and Stringifies YAML objects |
| `@scritch/hashes`     | Common crypto hashes |
