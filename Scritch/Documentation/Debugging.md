...
___
**<p align=center>Debugging is only available in dev builds, in Scritch version 1.2.0 and above.</p>**
___

## Getting a dev build

To use debugging tools, you need a dev build of Scritch. This is a restriction of JavascriptCore (the thing that runs scripts) that I have not yet found a way to avoid.
...
- Download the Scritch source code from Github or clone it locally using git. You can find a compressed version of the source code on the [releases page](https://github.com/Amwam/Scritch/releases/).

- In the source code, find and open `Scritch.xcodeproj` with Xcode (it's in the `Scritch` folder, surprisingly).

- On the top left corner of the Xcode window, hit the play button. The dev build should start.

## Preparing the debugger

To see your script in the debugger, you'll need to run that script at least once. It does not matter if it doesn't work, Scritch just needs to know you want to use it.
...
Once set up, you should be able to see your script in the `Develop` menu, in the submenu with the same name as your computer (in the screenshot, it's `Ivan MKII` - yours will be different, most likely), under the `Scritch Section`. Select the script you'd like to debug. 
...
The debugger is the same as the built-in one from Safari, and works just like any in-browser inspector. The central panel shows the source of your script. *Editing it there will not affect the script within Scritch, and any change will be lost.*
...
This is how Scritch makes sure modules are safe and compatible. The wrapper is added at runtime. It ain't pretty but it gets the job done.
...
While the breakpoint is active, Scritch will become unresponsive. To make Scritch usable again, continue the script execution by pressing the play button at the top left of the debugger.

## Reloading scripts

When reloading scripts within Scritch, the debugger **will not** automatically reload. If you want to check a new version of your script, close the debugger, run the scripts again for it to register, then re-open the debugger.
